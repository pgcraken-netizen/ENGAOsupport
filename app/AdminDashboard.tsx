...
const MEAL_LABEL: Record<Meal, string> = {
  full: "全量完食", normal: "普通量", half: "半量以下", none: "摂取なし",
};
const CONDITION_LABEL: Record<Condition, string> = {
  normal: "良好", slightly_poor: "やや不良", poor: "不良",
};
const MEAL_COLOR: Record<Meal, string> = {
  full: "#2563eb", normal: "#4a8c5c", half: "#d97706", none: "#6b7280",
};
const CONDITION_COLOR: Record<Condition, string> = {
  normal: "#4a8c5c", slightly_poor: "#d97706", poor: "#dc2626",
};
...
function exportCSV(records: RecordRow[], range: DateRange) {
  ...
  a.href = url;
  a.download = `えんがお記録_${label}_${date}.csv`;
  a.click();
...
export default function AdminDashboard() {
  ...
  async function fetchRecords() {
    ...
    const { data } = await supabase
      .from("records")
      .select("id, meal, condition, note, recorded_at, residents(name), staff(name)")
  ...
  const mealCounts = MEALS.reduce((acc, m) => {
    acc[m] = records.filter(r => r.meal === m).length;
    return acc;
  }, {} as Record<Meal, number>);

  const conditionCounts = CONDITIONS.reduce((acc, c) => {
    acc[c] = records.filter(r => r.condition === c).length;
    return acc;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8f9fa", fontFamily: "-apple-system, BlinkMacSystemFont, 'Hiragino Sans', sans-serif" }}>
      ...
      <header className="no-print" style={{
        background: "#4a8c5c", padding: "14px 24px",
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px 60px" }}>
        ...
        <div className="print-only" style={{ marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 13, color: "#555", margin: 0 }}>
            期間：{rangeLabel}（{formatDateLabel(dateRange)}）　出力日：{new Date().toLocaleDateString("ja-JP")}
          </p>
        </div>
        ...
        <div className="no-print" style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {DATE_OPTIONS.map(opt => (
            <button
              ...
              style={{
                padding: "8px 20px",
                background: dateRange === opt.value ? "#4a8c5c" : "#fff",
                color: dateRange === opt.value ? "#fff" : "#444",
                border: `1px solid ${dateRange === opt.value ? "#4a8c5c" : "#ddd"}`,
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: 60 }}>
            ...
            <p style={{ color: "#888", marginTop: 12 }}>読み込み中...</p>
          </div>
        ) : records.length === 0 ? (
          ...
          <>
            ...
            <section style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14 }}>
                ...
                {/* 体調集計 */}
                <div style={cardStyle}>
                  <p style={cardLabelStyle}>体調の内訳</p>
                  {CONDITIONS.map(c => (
                    <div key={c} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                      <span style={{ fontSize: 13, color: CONDITION_COLOR[c], fontWeight: 600 }}>
                        {CONDITION_LABEL[c]}
                      </span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: "#333" }}>
                        {conditionCounts[c]} 件
                        <span style={{ fontSize: 11, color: "#888", marginLeft: 4 }}>
                          ({records.length > 0 ? Math.round(conditionCounts[c] / records.length * 100) : 0}%)
                        </span>
                      </span>
                    </div>
                  ))}
                ...
                <StatCard
                  label="要注意（体調不良）"
                  value={`${conditionCounts.poor + conditionCounts.slightly_poor} 件`}
                  color={conditionCounts.poor > 0 ? "#dc2626" : "#d97706"}
                  sub={conditionCounts.poor > 0 ? `うち「不良」${conditionCounts.poor}件` : "「不良」なし"}
                />
              </div>
            </section>
                  ...
                  <tbody>
                    {residentStats.map((s, i) => (
                      <tr key={s.name} style={{ background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                        <Td bold>{s.name}</Td>
                        <Td center bold>{s.total}</Td>
                        {MEALS.map(m => <Td key={m} center>{s.meal[m] ?? 0}</Td>)}
                        {CONDITIONS.map(c => (
                          <Td key={c} center color={(s.condition[c] ?? 0) > 0 && c !== "normal" ? CONDITION_COLOR[c] : undefined}>
                            {s.condition[c] ?? 0}
                          </Td>
                        ))}
                      </tr>
            ...
            {/* ── 詳細記録一覧 ── */}
            <section>
              <SectionTitle text={`詳細記録一覧（${records.length}件）`} />
              <div style={{ overflowX: "auto" }}>
                <table style={tableStyle}>
                  <thead>
                    <tr style={{ background: "#f0f0f0" }}>
                  ...
                  <tbody>
                    {records.map((r, i) => (
                      <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                        <Td nowrap>{formatDateTime(r.recorded_at)}</Td>
...
function SectionTitle({ text }: { text: string }) {
  return (
    <h2 style={{ fontSize: 15, fontWeight: 700, color: "#333", margin: "0 0 12px", display: "flex", alignItems: "center", gap: 8 }}>
      <span style={{ display: "inline-block", width: 4, height: 16, background: "#4a8c5c", borderRadius: 2 }} />
      {text}
    </h2>
  );
}
...
function StatCard({ label, value, color, sub }: { label: string; value: string; color: string; sub?: string }) {
  return (
    <div style={cardStyle}>
      ...
      <p style={{ fontSize: 28, fontWeight: 800, color, margin: "4px 0 0" }}>{value}</p>
...
function Th({ children, color }: { children: React.ReactNode; color?: string }) {
  return (
    <th style={{ padding: "9px 12px", fontSize: 12, fontWeight: 700, textAlign: "left", whiteSpace: "nowrap", color: color ?? "#555", borderBottom: "2px solid #ddd" }}>
      {children}
    </th>
  );
}

function Td({ children, bold, center, color, nowrap }: {
  children: React.ReactNode; bold?: boolean; center?: boolean; color?: string; nowrap?: boolean;
}) {
  return (
    <td style={{
...
const cardLabelStyle: React.CSSProperties = {
  fontSize: 12, fontWeight: 600, color: "#888", margin: 0, textTransform: "uppercase", letterSpacing: "0.05em",
};
